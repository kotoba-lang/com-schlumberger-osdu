(ns schlumberger_osdu.main
  "Kotodama WASM entrypoint for the Schlumberger_osdu clean-room actor (L4) — Clojure port.
  ...
  facts (`schlumberger_osdu.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "schlumberger_osdu")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"schlumberger_osdu.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "schlumberger_osdu-compat" :tier tier :entities entities} 200])
...
(defn start! [] :schlumberger_osdu-compat/ready)
