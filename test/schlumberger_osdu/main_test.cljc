(ns schlumberger_osdu.main-test
  "Contract + behavioral test for the schlumberger_osdu-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "schlumberger_osdu-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'schlumberger_osdu.main-test)]
